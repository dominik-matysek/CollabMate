...
function Users() {
    ...
    },
    {
      ...
      render: (teams) => {
        if (!teams || teams.length === 0) {
          return <span style={{ color: "#1677ff" }}>Brak zespołów</span>;
        }

        // Menu for dropdown
        const menu = teams.map((team, index) => index > 0 && team.name);

        return (
          <Dropdown
            menu={{ menu }}
            dropdownRender={(menu) => <Menu items={menu} />}
            trigger={["click"]}
          >
            <a onClick={(e) => e.stopPropagation()}>
              {`${teams[0].name}`} <DownOutlined />
            </a>
          </Dropdown>
        );
      },
  ...
  return user.role === "ADMIN" ? (
    <div>
      <CustomTable columns={columns} data={users} onRowClick={onRowClick} />
    </div>
  ) : (
    <Navigate to="/" />
  );
